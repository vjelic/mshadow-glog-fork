...
__global__ void MapPlanKernel( hipLaunchParm lp,DstPlan dst, index_t xstride,
...
template<typename Saver, int block_dim_bits, int grid_size,
         typename DstPlan, typename Plan>
__global__ void MapPlanLargeKernel( hipLaunchParm lp,DstPlan dst, index_t xstride,
...
__global__ void
__launch_bounds__( hipLaunchParm lp,kMemUnit*kMemUnit, 1)
...
template<typename Saver, typename Reducer, int block_dim_bits,
         typename DType, typename DstPlan, typename Plan>
__global__ void MapReduceKeepDim1Kernel( hipLaunchParm lp,DstPlan dst, Plan plan, DType scale, Shape<4> pshape) {
...
__global__ void GetBatchedViewKernel( hipLaunchParm lp,DType **dst, DType *src, int num, int stride) {
...
__global__ void SoftmaxGradKernel( hipLaunchParm lp,DstPlan dst, SrcPlan1 src, SrcPlan2 label, index_t xmax) {
...
__global__ void SoftmaxKernel( hipLaunchParm lp,DstPlan dst, SrcPlan src, index_t xmax) {
  const unsigned x_size = 1 << x_bits;
  const int y = hipBlockIdx_x;
...
__global__ void Softmax3DGradKernel( hipLaunchParm lp,Tensor<gpu, 3, DType> dst,
...
template<int n_bits, typename DType>
__global__ void Softmax3DKernel( hipLaunchParm lp,Tensor<gpu, 3, DType> dst,
                    const Tensor<gpu, 3, DType> src) {
  const index_t xmax = dst.size(1);
  const index_t nmax = dst.size(2);
  const unsigned n_size = 1 << n_bits;
...
template<int x_bits, typename DType, typename DstPlan, typename SrcPlan1, typename SrcPlan2>
__global__ void AddTakeGradKernel( hipLaunchParm lp,DstPlan dst,
...
__global__ void IndexFillKernel( hipLaunchParm lp,DstPlan dst,
