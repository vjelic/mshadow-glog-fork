...
__global__ void MapPlanKernel(DstPlan dst, index_t xstride,
  ...
  MapPlanProc<Saver, DstPlan, Plan, block_dim_bits>
      (dst, xstride, dshape, plan, hipBlockIdx_x);
}
...
__global__ void MapPlanLargeKernel(DstPlan dst, index_t xstride,
  ...
  MapPlanProc<Saver, DstPlan, Plan, block_dim_bits>
      (dst, xstride, dshape, plan, hipBlockIdx_x + i * grid_size);
  }
}
...
__global__ void MapReduceKeepDim1Kernel(DstPlan dst, Plan plan, DType scale, Shape<4> pshape) {
  ...
  const int c = hipBlockIdx_x + hipBlockIdx_y * hipGridDim_x;
...
inline void GetBatchedView(DType **dst, DType *src, int num, int stride,
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(GetBatchedViewKernel<kBaseThreadBits, DType>), dim3(dimGrid), dim3(dimBlock), 0, stream_, dst, src, num, stride);
...
inline void SmoothSoftmaxGrad(const Tensor<gpu, 2, DType> &dst,
  ...
  CheckLaunchParam(dimGrid, dimBlock, "SoftmaxGrad");
  hipStream_t stream = Stream<gpu>::GetStream(dst.stream_);
  hipLaunchKernelGGL((SmoothSoftmaxGradKernel<kBaseThreadBits, DType>), dim3(dimGrid), dim3(dimBlock), 0, stream, expr::MakePlan(dst),
       expr::MakePlan(src),
       expr::MakePlan(label),
       dst.size(1),
       alpha);
...
__global__ void IndexFillKernel(DstPlan dst,
                                ...
                                const int ymax,
                                const int xmax) {
  int bid = hipBlockIdx_y * hipBlockDim_x + hipBlockIdx_x;
  int tid = bid * hipBlockDim_x * hipBlockDim_y + hipThreadIdx_y * hipBlockDim_x + hipThreadIdx_x;
  if (tid < ymax * xmax) {
    int i = tid / xmax;
    int j = tid % xmax;
